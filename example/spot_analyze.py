...
from mexc.client.api import ApiClient
from mexc.methods.api.marked_data_endpoints import MarkedDataEndpoints
from mexc.methods.api.spot_accounts_trade import SpotAccountsTrade
...
async def main() -> None:
    client = ApiClient()
    marked = MarkedDataEndpoints(client)
    spot = SpotAccountsTrade(client)
    pair_symbols: list[str] = []
    price_symbols: list[list[str]] = []
    volume_symbols: list = []

    print(f"MEXC spot analyze", end="\n\n")
    print(f"Settings",
    ...
    symbols: list[str] = (await marked.api_default_symbol()).get("data")
    ...
    for count, symbol in enumerate(pair_symbols, start=1):
        for symbol_price in symbol_prices:
            _symbol = symbol_price.get("symbol")
            _price = float(symbol_price.get("price"))
            if symbol == _symbol:
                if _price >= MIN_PRICE:
    ...
    for count, pair_symbol in enumerate(price_symbols, start=1):
        ...
        for trade_symbol in trades:
            _symbol = trade_symbol.get("symbol")
            _volume = float(trade_symbol.get("volume"))
            if _symbol == symbol:
